...
savefile <- tclvalue(
    tkgetSaveFile(
        filetypes = "{{PDF Files} {.pdf}} {{Windows Meta Files} {.emf}} {{PNG Files} {.png}}",
...
bk_a <- use_alpha
bk_w <- use_weight_as_width
saving_file <- 1

{
	if (regexpr("\\.pdf$", savefile, ignore.case=T, perl=T) > -1){
		library(Cairo)
		cairo_pdf(file=savefile, height = 8, width = 10.664, pointsize=12)
	}
	else if (regexpr("\\.emf$", savefile, ignore.case=T, perl=T) > -1){
		use_alpha <- 0
		use_weight_as_width <- 0
		win.metafile(filename=savefile, width = 10.664, height = 8, pointsize=12)
	}
	else if (regexpr("\\.png$", savefile, ignore.case=T, perl=T) > -1){
		library(Cairo)
		Cairo(width=853, height=640, unit="px", file=savefile, type="png", bg="white", dpi=72)
	}
	else {
		stop("Extension mismatch...")
	}
}

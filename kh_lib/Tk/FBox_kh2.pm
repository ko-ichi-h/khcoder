...
sub Populate {
	...
	use Encode;
	use Encode::Locale;
	$w->{'encoding'} = $w->getEncoding('locale_fs');
...
#       my $sortcmd = $w->cget(-sortcmd);
	...
	foreach my $f (sort $sortcmd readdir(FDIR)) {
	    $f = $w->_decode_filename($f); # KH Coder
...
sub ListBrowse {
	...
	my $t_gui = $text->[0];
	$t_gui = $w->_decode_filename($t_gui);
...
sub Done {
	...
	if ($w->cget('-multiple')) {
		...
		$path = $w->_decode_filename($path);
		$f    = $w->_decode_filename($f   );
	    $selectFilePath = JoinFile($path,$f);
	}
...
sub _decode_filename {
    my($w, $filename) = @_;
    $filename = $w->{encoding}->decode($filename) unless utf8::is_utf8($filename);
    return $filename;
}

...
sub calc_code_plugin{
	...
	if ($self->{radio} == 1){
		...
		while ($n <= $max){
			...
			#旧バージョンはMySQLのエンコードがeuc-jp
			$names{$n} = Encode::decode('euc-jp', $names{$n});
			if (length($names{$n})){
				$names{$n} =~ s/"/ /g;
				$headings .= "\"$names{$n}\",";
			}
...
sub make_plot_plugin{
	...
	$r_command .= "name_dim <- '成分'\n"; # 成分
	$r_command .= "name_eig <- '固有値'\n"; # 固有値
	$r_command .= "name_exp <- '寄与率'\n"; # 寄与率
...
if (plot_mode == "color"){
	...
	#X軸の左端を0、右端を1とし各単語がどこにいるかを割合で表すための計算
